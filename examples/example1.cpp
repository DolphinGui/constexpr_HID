...
template <typename T> using raw_array = std::array<uint8_t, sizeof(T)>;
...
constexpr static std::array<uint8_t, 85> joystick_report_desc = {
...
static std::array<uint8_t, 99> multitouch_report_desc = {
...
auto joystick_descriptor = descriptor(
    ...
    collection(
        collection_type::application, logical_extent(0_u8, 1_u8),
        ...
        logical_extent(0_u8, 7_u8), physical_extent(0_u8, uint16_t(315)),
        report_type(4_u8, 1_u8), raw<2>{0x65, 0x14},
        ...
        collection(
            ...
            input<io8>(io8{.array_or_variable = IOF::p2::variable})),
        logical_extent(0_u8, uint16_t(1023)), report_type(10_u8, 2_u8),
...
auto multitouch_descriptor = descriptor(
    ...
    collection(
        collection_type::application, usage(uses::digitizer::Finger),
        collection(collection_type::logical, usage(uses::digitizer::Tip_Switch),
                   ...
                   internal::logical_maximum<int16_t>(32767), uint8_t(0x65),
                   uint8_t(0x00), // units, none
                   report_type(uint8_t(16), uint8_t(2)),
                   input<io8>(io8{.array_or_variable = IOF::p2::variable})),
        usage_page(uses::page_type::digitizer),
        internal::logical_maximum<int32_t>(65535),
        report_type(uint8_t(16), uint8_t(1)), usage(uses::digitizer::Scan_Time),
        input<io8>(io8{.array_or_variable = IOF::p2::variable}),
        usage(uses::digitizer::Contact_Count), internal::logical_maximum<>(127),
        report_type(uint8_t(8), uint8_t(1)),
        input<io8>(io8{.array_or_variable = IOF::p2::variable}),
        usage_page(uses::page_type::digitizer),
        usage(uses::digitizer::Contact_Count_Maximum),
        internal::logical_maximum<>(10), report_type(uint8_t(8), uint8_t(1)),
...
int main() {
  ...
  test(multitouch_report_desc, multitouch_descriptor,
       "touchscreen is different");
